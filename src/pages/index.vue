...
<template>
  ...
  <form
    v-if="mauTambah"
    class="p-2 relative bg-orange-200 block"
    @submit.prevent="simpanDataBaru"
    action=""
  >
    <span
      @click="mauTambah = false"
      class="absolute top-0 right-2 text-gray-500 cursor-pointer"
      >&times;</span
    >
    ...
    <textarea
      required
      v-model="dataBaru"
      class="block text-sm w-full p-1"
      name=""
      id=""
      cols="30"
      rows="10"
    ...
    <input
      type="submit"
      class="bg-green-500 rounded text-white px-3 py-1 mt-2 inline-block cursor-pointer"
      value="Process"
    />
  </form>

  <div
    class="p-2 px-3 py-1 bg-pink-200 grid grid-cols-1 gap-1 overflow-auto max-w-[224px] max-h-50"
  >
    <div class="text-sm" v-for="x in data">
      {{ x }}
      <span
        @click="hapus(x)"
        class="underline cursor-pointer hover:text-pink-500"
        >(done)</span
      >
    </div>
  </div>
  ...
  <form action="" @submit.prevent="tambah" class="p-2">
    <input
      type="text"
      v-model="inputan"
      class="w-full p-1 text-sm border border-blue-500 caret-pink-500 text-pink-700 focus:border-pink-500 focus:outline-none"
